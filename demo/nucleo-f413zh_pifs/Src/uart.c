...
void UART_printf_(const char * fmt, ...)
{
    ...
    va_end(valist);
    HAL_UART_Transmit(&huart3, (uint8_t*) printf_buf, strlen(printf_buf),
            HAL_MAX_DELAY);
}
...
int _read(int file, char *buf, int len)
{
    ...
    if (printf_mutex)
    {
        ...
        if (osMutexWait(printf_mutex, osWaitForever) == osOK)
        {
            ...
            HAL_UART_Transmit(&huart3, buf, 1, HAL_MAX_DELAY); /* Echo */
            buf++;
            cntr++;
            while (HAL_UART_Receive(&huart3, buf, 1, 0) == HAL_OK && cntr < len)
            {
                HAL_UART_Transmit(&huart3, buf, 1, HAL_MAX_DELAY); /* Echo */
                buf++;
                cntr++;
