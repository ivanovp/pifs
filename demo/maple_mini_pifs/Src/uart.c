...
void UART_printf(const char * fmt, ...)
{
    ...
    if (printf_mutex && osMutexWait(printf_mutex, osWaitForever) == osOK)
    {
        ...
        vsnprintf(printf_buf, sizeof(printf_buf), fmt, valist);
        va_end(valist);
        HAL_UART_Transmit(&huart2, (uint8_t*) printf_buf, strlen(printf_buf),
                HAL_MAX_DELAY);
        if (printf_mutex)
        {
...
uint8_t UART_getchar(void)
{
    ...
    if (HAL_UART_Receive(&huart2, &ch, sizeof(ch), HAL_MAX_DELAY) == HAL_OK)
	{
		return ch;
	}
...
int _read(int file, char *buf, int len)
{
    ...
    if (printf_mutex)
    {
        ...
        if (osMutexWait(printf_mutex, osWaitForever) == osOK)
        {
            ...
            HAL_UART_Transmit(&huart2, buf, 1, HAL_MAX_DELAY); /* Echo */
            buf++;
            cntr++;
            while (HAL_UART_Receive(&huart2, buf, 1, 0) == HAL_OK && cntr < len)
            {
                HAL_UART_Transmit(&huart2, buf, 1, HAL_MAX_DELAY); /* Echo */
                buf++;
                cntr++;
