...
\item nucleo-f413zh\_pifs: Demo running on Nucleo-F413ZH board. There is no
flash memory on the board, some prototype hardware should be created.
It has internal debugger and virtual serial port. 
It can be compiled with GNU make and GCC or with STM32 System Workbench.
\item stm32\_f4ve\_pifs: Demo running on STM32-F4VE board. There is
W25Q16DV NOR flash soldered on the board, but it has not got UART. So external
debugger and USB-serial converter is needed.
It can be compiled with GNU make and GCC or with STM32 System Workbench.
\end{enumerate}
