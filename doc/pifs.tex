...
\tikzstyle{page} = [draw,fill=blue!15,minimum size=2em]
...
%\node[block] (at 0,0) (block1) {Block 1};
% Draw blocks
\foreach \x in {0,1,2,3} {
    ...
    \draw[loosely dotted] (page\x3) -- (page\x\y);
    \begin{pgfonlayer}{background2}
    % Draw block
    \node[fit=(page\x0) (page\x\y), dashed,fill=gray!30,draw,inner sep=0.45cm] (block\x){};
    \node[below right] at (block\x.north west) {Block \x};
    \end{pgfonlayer}
}
\begin{pgfonlayer}{background}
\node[fit=(block0) (block3),draw,inner sep=0.45cm] (flash){};
\node[below right] at (flash.north west) {Flash memory};
\end{pgfonlayer}
\end{tikzpicture}
\caption{Flasy memory layout}
\end{figure}
