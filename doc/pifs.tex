...
\tikzstyle{block} = [draw,fill=yellow!20,dashed,minimum size=2em]
\tikzstyle{page} = [draw,fill=brown!25,dashed,minimum size=2em]
...
\foreach \x in {0,1,2} {
    ...
    % Draw block
    \node[fit=(page\x0) (page\x\y), block,inner sep=0.45cm] (block\x){};
    \node[below right] at (block\x.north west) {Block \x};
    \end{pgfonlayer}
}
%%% Draw block n
\def\x{4}
...
\node[below right] at (block\x.north west) {Block n};
\end{pgfonlayer}

\begin{pgfonlayer}{background}
\node[fit=(block0) (block4),draw,dashed,fill=gray!15,inner sep=0.45cm] (flash){};
